...
def backfill(asset_ids: dict, timeframe_id:int, database: Database, exchange: BinanceClient)-> None:
   
   for asset, asset_id in asset_ids.items():
        validator = CandleValidator()
        last_timestamp = database.get_last_candle_timestamp(asset_id, timeframe_id)
        since = int(last_timestamp.timestamp() * 1000) + 1 
        while True:
            candles = exchange.fetch_candles(asset, "1m", since = since)
            if candles:
...
async def main():
    ...
    try:
        ...
        backfill(asset_ids, timeframe_id, database, exchange)
